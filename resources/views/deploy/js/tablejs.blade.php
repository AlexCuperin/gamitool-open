<script>
    ...
    function create_table(rows, columns){
        ...
        var trow = '';
        for(j=1; j<=columns; j++) {
            trow += '<th style="border: 1px solid #e9ecef">Module ' + j + '</th>';
        }
    ...
    function populate_table(resources, gld_id) {
        ...
        resources.forEach(function (elem, index) {
            ...
            if ((condition === true) || (reward === true)){
                $('#' + cell_id).append('<select class="'+elem.resource_type.name.replace(" ","_")+' width-100"></select>');
            }
